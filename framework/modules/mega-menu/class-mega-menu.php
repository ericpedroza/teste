...
class ZnMegaMenu{
	...
	function __construct(){
		...
		if( ZN()->is_request( 'admin' ) ){
			require(FW_PATH .'/modules/mega-menu/class-mega-menu-admin.php');
		}
		...
		foreach ($menu_items as $key => $menu_item) {
			...
			if( $mm_active && $mm_smart_area ){
				...
				if( ! empty( $pb_data ) ){
					...
					ZNPB()->register_smart_area( $mm_smart_area );
		...
		if ( $depth === 0 && $this->mm_active ) {
			...
			if( $this->mm_smart_area ){
				...
				echo '<div class="zn-megaMenuSmartArea-content">';
					ZN()->pagebuilder->zn_render_uneditable_content( $pb_data, $this->mm_smart_area );
				echo '</div>';
				$output .= ob_get_clean();
