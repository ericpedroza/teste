	...
	$.ZnAboutJs.prototype = {
		zinit : function() {
			...
			fw.filesystemModal.on( 'submit', 'form', function( event ) {
				...
				fw.do_plugin_action( fw.activePluginElement );
		...
		do_plugin_action( activePlugin ){

			// Perform the ajax call based on action
			var config = {};
				config.button			= activePlugin;
