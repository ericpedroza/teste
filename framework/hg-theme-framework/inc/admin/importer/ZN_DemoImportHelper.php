	...
	public static function getDemoConfig( $demoDirPath )
	{
		...
		if(is_file($filePath)){
			if(is_readable($filePath)) {
				$data = json_decode( trim( file_get_contents( $filePath ) ), true );
				if ( empty( $data ) ) {
					// maybe no data
					return null;
				}
...
//<editor-fold desc="::: UTILITY FUNCTIONS">
	...
	public static function log( $error = '', $className = null )
	{
		if(! empty($error))
		{
			...
			file_put_contents(ABSPATH.'/import.log', '['.$className.']: '.$error.PHP_EOL, FILE_APPEND);
		}
	}

	public static function clearLogFile(){
		file_put_contents(ABSPATH.'/import.log', '');
	}
//</editor-fold desc="::: UTILITY FUNCTIONS">
...
//<editor-fold desc="::: IMPORT FUNCTIONS">
	...
	public static function importThemeOptions( $demoDirPath )
	{
		...
		if(empty($images))
		{
			...
			if(! empty($themeOptions)){
				...
				$optionName = ZN()->theme_data['options_prefix'];
		...
		if( $replaceImages )
		{
			...
			foreach( $images as $imgPlaceholder => $entry ) {
				if ( isset( $entry['path'] ) )
				{
					...
					{
						$attID = self::__importImage( $uploadsDir.$entry['path'], $entry['path'] );
			...
			foreach( $posts as $oldPostID => $postInfo )
			{
				...
				$newPostID = post_exists($post_db['post_title']);
				if( ! empty($newPostID)){
	...
	public static function installDependentPlugins( $demoDirPath )
	{
		...
		if(empty($demoName)){
			self::log('Demo name could not be retrieved from path: '.$demoDirPath.'. Skipping this step.');
			return false;
		}

		//#! Get plugins from demo's config file
		$availableDemos = ZN_HogashDashboard::getAllDemos();
		if( empty($availableDemos) ){
			return true;
		}
		elseif(is_array($availableDemos) && isset($availableDemos['error'])){
			return true;
		}

		$dependentPlugins = array();
		foreach($availableDemos as $demo_name => $info)
		{
			if( strcasecmp($demoName, $demo_name) == 0)
			{
				$dependentPlugins = $info['plugins'];
			}
		}
		...
		$znAddonsMgr = ZnAddonsManager();

		// Loop through the list and install plugins and their options
		foreach ( $dependentPlugins as $slug )
		{
			$result = true;

			// Check to see if the plugin is already installed
			if( $znAddonsMgr->is_plugin_installed( $slug ) ){
				if( ! $znAddonsMgr->is_plugin_active( $slug ) ){
					// Try to activate the plugin
					$result = $znAddonsMgr->do_plugin_activate( $slug );
				}
			}
			else {
				// Install the plugin and activate it
				$result = $znAddonsMgr->do_plugin_install( $slug );

				// Invalidate WP plugins cache so our new installed plugins will be included in it when WP will
			...
			if ( is_array( $result ) && isset( $result['error'] ) ) {
				self::log("Error installing/activating plugin [{$slug}]: ". $result['error'] );
			}
			// Install options
			else {
				$hasOptions = (isset($allPluginsOptions[$slug]) && !empty($allPluginsOptions[$slug]));

				if ( $hasOptions )
				{
					// Get options
					$pluginOptions = $allPluginsOptions[$slug];

					// Check for revslider since it requires a special attention
					if( 'revslider' == $slug )
					{
						// Do nothing, sliders will be installed in installRevolutionSliders()
						continue;
					}

					// Other plugins, install options
					foreach( $pluginOptions as $i => $row )
					{
						// Ignore permalinks since they've already been set
						if( 'woocommerce_permalinks' != $row['option_name'])
						{
							$value = $row['option_value'];
							if( 'woocommerce_shop_page_id' == $row['option_name']){
								if(isset($tempData['posts'][$value])){
									$value = $tempData['posts'][$value];
								}
							}

							// Set option
							update_option( $row['option_name'], $value, $row['autoload'] );
						}
					}
				}
			}
	...
	public static function installRevolutionSliders( $demoDirPath )
	{
		...
		if(! empty($demoDirPath))
		{
			...
			{
				// Get options
				$pluginOptions = $allPluginsOptions['revslider'];

				// Install all sliders if any
				if( ! empty($pluginOptions) ){
					foreach( $pluginOptions as $sliderName ) {
	...
	public static function importDemoImages( $demoDirPath )
	{
		...
		foreach($imagesMap as $placeholder => $info )
		{
			...
			// Process image
			if( false === self::__importImage( $fullPath, $path ) ){
	 ...
	 */
	private static function __importImage( $filePath, $internalPath )
	{
	...
	public static function  postProcessing( $demoDirPath )
	{
		...
		if(! empty($tempData))
		{
			...
			if (isset($tempData['posts']) && ! empty($tempData['posts']))
			{
				...
				// Update the Shop Page ID if woocommerce exists
				...
				if( ! empty($oldShopPageID)){
					if(isset($tempData['posts'][$oldShopPageID])){
...
//</editor-fold desc="::: IMPORT FUNCTIONS">
	...
	public static function _rewrite_urls(){
		add_option( 'znhg_flush_rewrite_rules', true );
	}
