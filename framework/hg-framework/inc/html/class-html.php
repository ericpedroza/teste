<?php if(! defined('ABSPATH')){ return; }
	...
	class ZnHgFw_Options {
		...
		function registerStandardOptionTypes(){
			...
			if( is_array( $files ) ){
				foreach ( $files as $file ) {
					...
					$this->registerOptionType( $optionType, require( $file ) );
				}
			}
		...
		function registerOptionType( $optionId, $optionClass ){
			$this->_optionTypes[$optionId] = $optionClass;
		}
		...
		function renderOption( $optionConfig ){
			...
			if( ! isset( $this->_optionTypes[$optionConfig['type']] ) ){
				...
				$message .= sprintf(__( 'The option type (%s) is missing or we don\'t have any knowledge of this option type', 'znhgfw_framework' ), $optionConfig['type']);
				$message .= '</div>';
				return $message;
			}

			return $this->_optionTypes[$optionConfig['type']]->render( $optionConfig );
		...
		function enqueue_scripts(){
			...
			wp_register_script( 'wp-color-picker-alpha', FW_URL .'/assets/js/wp-color-picker-alpha.js', array( 'wp-color-picker' ), ZNHGFW()->getVersion() );
			...
			wp_enqueue_script( 'zn_timepicker', FW_URL .'/assets/js/jquery.timepicker.min.js',array( 'jquery' ), ZNHGFW()->getVersion(), true );
			wp_enqueue_script( 'zn_modal', FW_URL .'/assets/js/zn_modal.js',array( 'jquery' ), ZNHGFW()->getVersion(),true );
			wp_enqueue_script( 'zn_media', FW_URL .'/assets/js/zn_media.js',array( 'jquery' ), ZNHGFW()->getVersion(),true );
			wp_enqueue_script( 'zn_ace', FW_URL .'/assets/js/src-min-noconflict/ace.js',array( 'jquery' ), ZNHGFW()->getVersion(),true );
			wp_enqueue_script( 'ZnFormHelper', FW_URL .'/assets/js/ZnFormHelper.js',array( 'jquery' ), ZNHGFW()->getVersion(),true );

			wp_enqueue_script( 'zn_html_script', FW_URL .'/assets/js/zn_html_script.js',array( 'jquery' ), ZNHGFW()->getVersion(),true );
			...
			// TODO : MOVE TO SHORTCODE MANAGER CLASS
			// Load the main shortcodes Scripts
			wp_register_script( 'znhg-options-machine', FW_URL .'/assets/js/dist/admin/options/options.min.js', array( 'backbone', 'wp-color-picker', 'wp-color-picker-alpha', 'jquery-ui-slider' ), ZNHGFW()->getVersion(), true );
			wp_register_style( 'znhg-options-machine', FW_URL .'/assets/css/options.css', array('wp-color-picker'), ZNHGFW()->getVersion() );
			// END OLD SCRIPTS
